...
test('seal(message, opts) is a function', (t) => {
  t.true('function' === typeof seal)
})

test('seal(message, opts) throws on bad input', (t) => {
...
test('seal(message, opts) seals a message', (t) => {
...
test('open(message, opts) is a function', (t) => {
  t.true('function' === typeof open)
  t.true(open === seal.open)
})

test('open(message, opts) throws on bad input', (t) => {
...
test('open(message, opts) opens a sealed message', (t) => {
  const { publicKey, secretKey } = keyPair()
  const message = Buffer.from('hello')
  const sealed = seal(message, { publicKey })
